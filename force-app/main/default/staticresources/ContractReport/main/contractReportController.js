$(document).ready(function(){
    ...
    $('[html-event="downloadCsv"]').prop('disabled', true);
    /*var filters = [];
...
function getDataFromApex( filters ) {
    ...
    getAllContractsAsFilters(filters, function(result){
        ...
        hideLoader();
        $('[html-event="downloadCsv"]').prop('disabled', false);
    });
}
...
function downloadCSV(e) {
    ...
    window.PageInstance.getAllData().forEach(function(row){
		...
		if (row.strAccountName) {
			csvRow.push(row.strAccountName.replace(/[^a-zA-Z0-9 ]/g, " "));
		} else {
			csvRow.push(row.strAccountName);
		}

		csvRow.push(row.OpportunityId);
		csvRow.push( row.QuoteNo );
		...
		if (row.ProductDescription) {
			row.ProductDescription.replace(/\n/g, " ");
			row.ProductDescription.replace(/\//g, " ");
            //csvRow.push( '"' +  + '"' );
            csvRow.push( '"' + row.ProductDescription.replace(/[^a-zA-Z0-9 ]/g, ' ') + '"' );
        } else {
            csvRow.push( '' );
        }
